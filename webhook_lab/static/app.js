...
let token = '', config = null, events = [], selected = null, offset = 0;
...
async function selectEvent(id) {
  ...
  for (const attempt of event.attempts) {
    ...
    meta.textContent = `${date(attempt.started_at)} · ${attempt.duration_ms ?? '—'} ms${attempt.error ? ` · ${attempt.error}` : ''}`;
...
$('login-form').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    $('endpoint').textContent = `${location.origin}${config.inbox_path}`;
    $('target').replaceChildren();
    for (const target of config.targets) {
      const option = document.createElement('option');
      option.value = target.id; option.textContent = target.name;
...
$('signout').addEventListener('click', () => {
  token = ''; config = null; selected = null; events = []; offset = 0;
  detailSequence++; listSequence++;
  $('event-detail').hidden = true; $('empty-detail').hidden = false;
  $('workspace').hidden = true; $('login').hidden = false;
  for (const id of ['payload', 'headers', 'attempts', 'endpoint', 'event-list', 'detail-id']) $(id).replaceChildren();
  $('token').focus();
});
$('sample').addEventListener('click', async () => {
  $('sample').disabled = true;
  try {
    const response = await fetch(config.inbox_path, {
    ...
    notify('Sample captured. Try a successful or failed replay.');
...
$('replay').addEventListener('click', async () => {
  ...
  try {
    ...
    notify(result.status === 'delivered' ? `Delivered · HTTP ${result.status_code}` : result.error);
  } catch (error) { report(error); }
  finally { $('replay').disabled = false; $('replay').textContent = '↻ Replay'; }
});
$('copy').addEventListener('click', async () => {
  try { await navigator.clipboard.writeText($('endpoint').textContent); notify('Private inbox URL copied.'); }
  catch { notify('Select the inbox URL and copy it manually.'); }
});
